    ...
    Object.defineProperties(CameraInteractor.prototype,
    {
        ...
        requestLiveView : {
            value: function(network, camera, listener)
            {
				...
				({
					type: "POST",
					url: "https://rest-" + credentials.region + "." + server + "/api/v5/accounts/" + credentials.account.account_id + "/networks/" + network + "/cameras/" + camera + "/liveview",
                    dataType: 'json',
					beforeSend: function(xhr) {                        
						xhr.setRequestHeader("TOKEN_AUTH", credentials.token);
					},
        ...
        joinCommand : {
            value: function(network, camera, command, listener)
            {
				...
				({
					type: "GET",
					url: "https://rest-" + credentials.region + "." + server + "/api/v5/accounts/" + credentials.account.account_id + "/networks/" + network + "/cameras/" + camera + "/join/" + command,
                    dataType: 'json',
					beforeSend: function(xhr) {                        
						xhr.setRequestHeader("TOKEN_AUTH", credentials.token);
					},
